...
export default function DashboardPage() {
  ...
  const filteredMatches = matches?.filter(
    (match) =>
      sportFilter === "all" ||
      match.sport.toLowerCase() === sportFilter.toLowerCase()
  );
  ...
  const handleSelectMatch = (match: any) => {
    scrollToSection("bet-history-section");
                ...
                {isLoadingMatches ? (
                  Array(3)
                    ...
                    .map((_, index) => (
                      <div
                        key={index}
                        className="bg-secondary rounded-xl overflow-hidden shadow-lg border border-gray-700 h-64 animate-pulse"
                      />
                    ))
